...
// .wrangler/tmp/bundle-QjwZdo/checked-fetch.js
...
// .wrangler/tmp/bundle-QjwZdo/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-QjwZdo/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
var checkOptionalParameter = (path) => {
  ...
  return results.filter((v, i3, a) => a.indexOf(v) === i3);
};
    ...
    for (let i3 = 0, len = paths.length; i3 < len; i3++) {
      ...
      Object.keys(routes).forEach((m2) => {
        if (method === METHOD_NAME_ALL || method === m2) {
          ...
          routes[m2][path2].push([handler, paramCount - len + i3 + 1]);
        }
      });
    }
  }
...
var Node2 = class {
  ...
  insert(method, path, handler) {
    ...
    const handlerSet = {
      handler,
      possibleKeys: possibleKeys.filter((v, i3, a) => a.indexOf(v) === i3),
      name: this.name,
      score: this.order
    };
...
// node_modules/@upstash/vector/dist/chunk-EEOSY6EV.mjs
init_checked_fetch();
init_modules_watch_stub();
...
var E = class {
  ...
  async request(e) {
    let t = { cache: this.options.cache, method: "POST", headers: this.headers, body: JSON.stringify(e.body), keepalive: true, signal: this.options.signal }, a = null, s = null;
    for (let M = 0; M <= this.retry.attempts; M++)
      try {
        a = await fetch([this.baseUrl, ...e.path ?? []].join("/"), t);
        break;
      } catch (A) {
        if (this.options.signal?.aborted) {
          let g = new Blob([JSON.stringify({ result: this.options.signal.reason ?? "Aborted" })]), P = { status: 200, statusText: this.options.signal.reason ?? "Aborted" };
          a = new Response(g, P);
          break;
        }
        s = A, await new Promise((g) => setTimeout(g, this.retry.backoff(M)));
      }
    if (!a)
      throw s ?? new Error("Exhausted all retries");
    let o = await a.json();
    if (!a.ok)
      throw new i(`${o.error}`);
    return { result: o.result, error: o.error };
  }
};
var n = class {
  payload;
...
var p = class extends n {
  constructor(e, t) {
    ...
    let s = [];
    Array.isArray(e) ? s.push(...e) : s.push(e), super(s, a);
  }
};
var f = class extends n {
  constructor(e, t) {
    let a = "query";
    a = e.some((o) => o.data) ? "query-data" : "query", t?.namespace && (a = `${a}/${t.namespace}`), super(e, a);
  }
};
...
var m = class extends n {
  constructor(e, t) {
    let a = "upsert";
    Array.isArray(e) ? a = e.some((o) => R(o)) ? "upsert" : "upsert-data" : a = R(e) ? "upsert" : "upsert-data", t?.namespace && (a = `${a}/${t.namespace}`), super(e, a);
  }
};
var R = (r) => "vector" in r;
...
var b = class {
  ...
  queryMany = (e, t) => new f(e, t).exec(this.client);
  ...
  info = () => new C().exec(this.client);
  listNamespaces = () => new x().exec(this.client);
  deleteNamespace = (e) => new T(e).exec(this.client);
};

// node_modules/@upstash/vector/dist/nodejs.mjs
var T2 = class i2 extends b {
  constructor(e) {
    if (typeof e < "u" && "request" in e) {
      super(e);
      return;
    }
    let n2 = e?.token ?? process.env.NEXT_PUBLIC_UPSTASH_VECTOR_REST_TOKEN ?? process.env.UPSTASH_VECTOR_REST_TOKEN, t = e?.url ?? process.env.NEXT_PUBLIC_UPSTASH_VECTOR_REST_URL ?? process.env.UPSTASH_VECTOR_REST_URL;
    if (!n2)
      throw new Error("UPSTASH_VECTOR_REST_TOKEN is missing!");
    if (!t)
      throw new Error("UPSTASH_VECTOR_REST_URL is missing!");
    (t.startsWith(" ") || t.endsWith(" ") || /\r|\n/.test(t)) && console.warn("The vector url contains whitespace or newline, which can cause errors!"), (n2.startsWith(" ") || n2.endsWith(" ") || /\r|\n/.test(n2)) && console.warn("The vector token contains whitespace or newline, which can cause errors!");
    let r = new E({ baseUrl: t, retry: e?.retry, headers: { authorization: `Bearer ${n2}` }, cache: e?.cache === false ? void 0 : e?.cache || "no-store", signal: e?.signal });
    super(r);
  }
  static fromEnv(e, n2) {
    let t = e?.UPSTASH_VECTOR_REST_URL || process?.env.UPSTASH_VECTOR_REST_URL, r = e?.UPSTASH_VECTOR_REST_TOKEN || process?.env.UPSTASH_VECTOR_REST_TOKEN;
    if (!t)
    ...
    return new i2({ ...n2, url: t, token: r });
  }
};
...
var arrOps = {
  add: function(arr, i3, document) {
    if (isInteger(i3)) {
      arr.splice(i3, 0, this.value);
    } else {
      arr[i3] = this.value;
    }
    return { newDocument: document, index: i3 };
  },
  remove: function(arr, i3, document) {
    var removedList = arr.splice(i3, 1);
    return { newDocument: document, removed: removedList[0] };
  },
  replace: function(arr, i3, document) {
    var removed = arr[i3];
    arr[i3] = this.value;
    return { newDocument: document, removed };
  },
...
function applyPatch(document, patch, validateOperation, mutateDocument = true, banPrototypeModifications = true) {
  ...
  for (let i3 = 0, length = patch.length; i3 < length; i3++) {
    results[i3] = applyOperation(document, patch[i3], validateOperation, true, banPrototypeModifications, i3);
    document = results[i3].newDocument;
  }
  ...
  try {
    ...
    if (document) {
      ...
      for (var i3 = 0; i3 < sequence.length; i3++) {
        externalValidator(sequence[i3], i3, document, void 0);
      }
    }
  } catch (e) {
...
function _areEquals(a, b2) {
  ...
  if (a && b2 && typeof a == "object" && typeof b2 == "object") {
    var arrA = Array.isArray(a), arrB = Array.isArray(b2), i3, length, key;
    if (arrA && arrB) {
      length = a.length;
      if (length != b2.length)
        return false;
      for (i3 = length; i3-- !== 0; )
        if (!_areEquals(a[i3], b2[i3]))
          return false;
      return true;
    }
    if (arrA != arrB)
...
var Client = class {
  ...
  async _loadChildRuns(run) {
    ...
    const treemap = {};
    const runs = {};
    childRuns.sort((a, b2) => (a?.dotted_order ?? "").localeCompare(b2?.dotted_order ?? ""));
...
function parseUnionDef(def, refs) {
  ...
  if (options.every((x2) => x2._def.typeName in primitiveMappings && (!x2._def.checks || !x2._def.checks.length))) {
    ...
    if (types.length === options.length) {
      const uniqueTypes = types.filter((x2, i3, a) => a.indexOf(x2) === i3);
      return {
...
var Graph = class {
  ...
  toJSON() {
    const stableNodeIds = {};
    Object.values(this.nodes).forEach((node, i3) => {
      stableNodeIds[node.id] = validate_default(node.id) ? i3 : node.id;
    });
    return {
...
var Runnable = class extends Serializable {
  ...
  _getOptionsList(options, length = 0) {
    ...
    if (length > 1 && !Array.isArray(options) && options.runId) {
      ...
      return Array.from({ length }, (_, i3) => ensureConfig(i3 === 0 ? options : subsequent));
    }
  ...
  async batch(inputs, options, batchOptions) {
    ...
    const batchCalls = inputs.map((input, i3) => caller.call(async () => {
      try {
        const result = await this.invoke(input, configList[i3]);
        return result;
      } catch (e) {
  ...
  async _batchWithConfig(func, inputs, options, batchOptions) {
    ...
    const runManagers = await Promise.all(callbackManagers.map(async (callbackManager, i3) => {
      const handleStartRes = await callbackManager?.handleChainStart(this.toJSON(), _coerceToDict2(inputs[i3], "input"), optionsList[i3].runId, optionsList[i3].runType, void 0, void 0, optionsList[i3].runName ?? this.getName());
      delete optionsList[i3].runId;
  ...
  async _batch(inputs, configs, runManagers, batchOptions) {
    ...
    try {
      await (0, import_p_retry3.default)(async (attemptNumber) => {
        const remainingIndexes = inputs.map((_, i3) => i3).filter((i3) => resultsMap[i3.toString()] === void 0 || // eslint-disable-next-line no-instanceof/no-instanceof
        resultsMap[i3.toString()] instanceof Error);
        ...
        let firstException;
        for (let i3 = 0; i3 < results.length; i3 += 1) {
          const result = results[i3];
          const resultMapIndex = remainingIndexes[i3];
          if (result instanceof Error) {
            if (firstException === void 0) {
              firstException = result;
              firstException.input = remainingInputs[i3];
            }
          }
    ...
    return Object.keys(resultsMap).sort((a, b2) => parseInt(a, 10) - parseInt(b2, 10)).map((key) => resultsMap[parseInt(key, 10)]);
  }
  async batch(inputs, options, batchOptions) {
...
var RunnableSequence = class extends Runnable {
  ...
  async batch(inputs, options, batchOptions) {
    ...
    const runManagers = await Promise.all(callbackManagers.map(async (callbackManager, i3) => {
      const handleStartRes = await callbackManager?.handleChainStart(this.toJSON(), _coerceToDict2(inputs[i3], "input"), configList[i3].runId, void 0, void 0, void 0, configList[i3].runName);
      delete configList[i3].runId;
      return handleStartRes;
    }));
...
var RunnableMap = class extends Runnable {
  ...
  async *_transform(generator, runManager, options) {
    ...
    const tasks = new Map(Object.entries(steps).map(([key, runnable], i3) => {
      const gen = runnable.transform(inputCopies[i3], patchConfig(options, {
        callbacks: runManager?.getChild(`map:key:${key}`)
      }));
  ...
  async batch(inputs, options, batchOptions) {
    ...
    for (const runnable of this.runnables()) {
      try {
        ...
        await Promise.all(runManagers.map((runManager, i3) => runManager?.handleChainEnd(_coerceToDict2(outputs[i3], "output"))));
        return outputs;
      } catch (e) {
        if (firstError === void 0) {
          firstError = e;
...
app.post("/", async (c2) => {
  ...
  try {
    ...
    let { message } = body;
    if (!message) {
      return c2.json({ error: "message is required" }, { status: 400 });
    }
    if (message.length > 1e3) {
      return c2.json({ error: "Message is too long" }, { status: 413 });
    }
    ...
    if (flaggedFor.size > 0) {
      const sorted = Array.from(flaggedFor).sort((a, b2) => a.score > b2.score ? -1 : 1)[0];
      return c2.json({
        isProfanity: true,
        score: sorted.score,
      ...
      const mostProfaneChunk = vectorRes.sort((a, b2) => a.score > b2.score ? -1 : 1)[0];
      return c2.json({
        isProfanity: false,
